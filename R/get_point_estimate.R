...
get_point_estimate <- function(
  ...
  s1_condi_c <- calculate_statistics(
    model = cox_censor,
    data = data,
    type = 1,
    trt = trt,
  ...
  s0_condi_c <- calculate_statistics(
    model = cox_censor,
    data = data,
    type = 0,
    trt = trt,
  ...
  if (clmq) {
    ...
    sim_dt <- Q(
      ...
      i_bh = c(1, 1, 2, 2),
      j_surv_cond = 1:4,
      const = list(M = M, cpp = cpp, bh_all = bh_all, s_all = s_all),
      n_jobs = 4,
      memory = memory,
      seed = seed,
      export = list(
      ...
      pkgs = c('survival', 'Rcpp')
    )

    output <- calculate_trt_effect(
      sim_out_1d = sim_dt[[1]],
      sim_out_0d = sim_dt[[2]],
      sim_out_1c = sim_dt[[3]],
      sim_out_0c = sim_dt[[4]]
...
.fx_clsmq_simcoun <- function(i_bh, j_surv_cond, M, cpp, bh_all, s_all) {
  out <- simulate_counterfactuals(
    bh = bh_all[[i_bh]],
    surv_cond = s_all[[j_surv_cond]],
    cpp = cpp,
    M = M
  )
  out['bh'] <- i_bh
