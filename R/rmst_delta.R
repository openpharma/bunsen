...
rmst_delta <- function(
  ...
  phi_c <- colSums(
    (gt_c * ht_c - cumsum(diff(c(0, cumhaz0$hazard))) * pi_c) *
      diff(c(cumhaz0$time, tau))
  )
  ...
  mat_vt_t <- matrix(
    0,
    nrow = length(cumhaz1$time),
    ncol = length(cumhaz1$time)
  )
  ...
  phi_t <- colSums(
    (gt_t * ht_t - cumsum(diff(c(0, cumhaz1$hazard))) * pi_t) *
      diff(c(cumhaz1$time, tau))
  )
  ...
  var_d <- as.numeric(
    omga_c /
      n0 +
      omga_t / n1 +
      (phi_t - phi_c) %*% sigma %*% (phi_t - phi_c) / fit$n +
      cov_tc / fit$n
  )
